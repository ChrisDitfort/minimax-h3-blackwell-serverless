...
The corrected immutable registry target is <code>multimodal-4</code> in <code>.github/workflows/build-dual-gpu.yml</code>. That tag is not an endpoint ID and must not be placed in a RunPod API URL. Repository source does not encode the RunPod dashboard display name. <code>multimodal-3</code> remains an immutable rollback/evidence image and contains the Ref2VA Autogrow defect documented in section 22; do not treat it as the corrected target.
...
The active Cloudflare <code>GET /capabilities</code> is a static legacy document. It reports the old quality table, omits <code>21:9</code>, and reports reference generation unavailable because it says the Ref2VA checkpoint is excluded. That result is stale for the complete model inventory and for the corrected <code>multimodal-4</code> target.
...
| Ref2VA Autogrow inputs used bare one-based keys | <code>multimodal-3</code> emitted <code>ref_image_1</code>. ComfyUI accepted the prompt, then <code>MiniMaxH3ReferenceToVideo.execute()</code> raised <code>TypeError</code> before sampling. | **FIXED in multimodal-4**: API graphs use dotted, zero-based V3 paths such as <code>ref_images.ref_image_0</code>, which ComfyUI normalizes into the node's <code>ref_images</code> dictionary. Image-build verification reads the actual pinned ComfyUI source. |
...
| Tag | <code>multimodal-4</code> |
| Digest | <code>sha256:a8f93afb6759c91bdb99ee0474aa34fb2fcf26135d5a21e1d5a44ff6cc62fb2f</code> |
| Source commit | <code>1f9035fb0c397509e3b20ed63b5336507ad98114</code> |
| Build ID | <code>33289814525-1</code> |
| Compressed size | <code>64,913,331,365 bytes</code> total compressed layers (<code>64.913 GB</code> decimal / <code>60.455 GiB</code>) |
...
| ComfyUI revision | <code>dec5d9450a5290bcf63430409ea41018e67f41c3</code> (runtime capability short form <code>dec5d945</code>; <code>v0.30.2</code>) |

Registry verification found 51 total layers: 43 code/base layers plus all eight model layers in <code>models.tsv</code>. The image config contains the exact source commit, image tag, build ID, <code>python3 /opt/serverless/handler.py</code> entrypoint, and <code>H3_GPU_MODE=single</code> default. CI verified every model layer in order and ran the Ref2VA contract check against ComfyUI <code>dec5d945</code> inside the image before publication. The prior <code>multimodal-3</code> tag remains at its distinct digest <code>sha256:7b7e40ba6ca2aeb5598a2eb01ab645ebf51bc659375084eda449e582dbe67751</code>.

## Ref2VA repair evidence and deployment gate

Both retained <code>multimodal-3</code> failures reached the same first failing operation:

| RunPod job | RunPod worker | Handler process | Comfy execution | Boundary |
| --- | --- | --- | ---: | --- |
| <code>badb0b0c-10b9-4fbf-aead-f4d6d220d8cc-u1</code> | <code>tokpswl2phu64q</code> | <code>169ea03ee729</code> | 2.05 seconds | Prompt accepted; node <code>#conditioning</code> / <code>MiniMaxH3ReferenceToVideo</code> rejected unexpected keyword <code>ref_image_1</code>; 0 sampling steps. |
| <code>6ac5543a-a8a2-4956-aa42-68f9bf21c9cb-u1</code> | <code>qa4siyqxrdtu45</code> | <code>dee69affd2a4</code> | 1.57 seconds | Same node and exception; 0 sampling steps. |

In both jobs, signed reference retrieval, PNG validation/Pillow decode, 1024x576 staging, graph construction, and <code>POST /prompt</code> acceptance completed. Video/audio VAEs and the text encoder began loading; execution failed when ComfyUI invoked the Ref2VA conditioning node, before the Ref2VA UNet load, scheduler, sampler, decode, or output. Cleanup ran after the terminal failure and callbacks reported only the safe downstream classification.

The corrected builder emits all four V3 Autogrow groups as dotted, zero-based API paths while retaining one-based H3 prompt ordinals. It also converts native <code>LoadVideo</code>'s <code>VIDEO</code> output through <code>GetVideoComponents</code> before supplying H3's <code>IMAGE</code> video-reference socket. <code>referenceFidelity=standard</code> remains <code>match</code>; Ref2VA Quality remains the base checkpoint at 20 steps with no Turbo LoRA. No canonical request field or capability value changed.

The image is published but was not assigned to endpoint <code>d0p4f4hgxyqsy2</code> by this repair task because endpoint mutation was not explicitly authorized. Therefore capabilities, the cheap FL2VA smoke, and the one-image Ref2VA production-shaped test against <code>multimodal-4</code> remain deployment validation gates. Do not report Ref2VA live success until the endpoint returns the build identity above and the one-image generation completes through output storage.
...
Local verification in the rebuilt worktree:
    ...
    python -m pytest -q
    478 passed, 43 skipped, 153 subtests passed
...
Immutable image publication and read-only registry verification:

    GitHub Actions run: 33289814525 (success)
    source commit:       1f9035fb0c397509e3b20ed63b5336507ad98114
    image:               ghcr.io/chrisditfort/minimax-h3-blackwell-serverless:multimodal-4
    digest:              sha256:a8f93afb6759c91bdb99ee0474aa34fb2fcf26135d5a21e1d5a44ff6cc62fb2f
    layers:              51 total = 43 code/base + 8 verified model layers
    compressed bytes:    64,913,331,365

The published config was read back from GHCR and contains the expected build identity, <code>python3 /opt/serverless/handler.py</code> entrypoint, and <code>H3_GPU_MODE=single</code> default. The image build itself executed <code>scripts/verify_ref2va_contract.py</code> successfully against the baked ComfyUI source. No RunPod endpoint was changed and no GPU generation was started.
